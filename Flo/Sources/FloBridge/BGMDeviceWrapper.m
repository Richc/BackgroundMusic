...
// Constants
NSString * const kFloAudioClientsChangedNotification = @"FloAudioClientsChangedNotification";
NSString * const kFloDeviceListChangedNotification = @"FloDeviceListChangedNotification";
NSString * const kFloVolumeChangedNotification = @"FloVolumeChangedNotification";
...
- (BOOL)connect {
    ...
    if (status != noErr) {
        NSLog(@"[Flo] Failed to get device list size: %d", (int)status);
        return NO;
    }
    ...
    if (status != noErr) {
        NSLog(@"[Flo] Failed to get device list: %d", (int)status);
        free(devices);
        return NO;
    }
    ...
    for (UInt32 i = 0; i < deviceCount; i++) {
        ...
        if (status == noErr && uidRef != NULL) {
            ...
            // Check for BGMDevice or Flo device UID
            if ([uid containsString:@"BGMDevice"] || 
                [uid containsString:@"FloDevice"] ||
                [uid containsString:@"Flo"]) {
                self.deviceID = devices[i];
                NSLog(@"[Flo] Connected to virtual device: %@ (ID: %u)", uid, devices[i]);
                free(devices);
                return YES;
            }
        }
    ...
    free(devices);
    NSLog(@"[Flo] Virtual audio device not found");
    return NO;
}
...
- (BOOL)setVolume:(float)volume forAppWithBundleID:(NSString *)bundleID {
    ...
    NSLog(@"[Flo] Set volume %.2f for %@", volume, bundleID);
...
- (BOOL)setMuted:(BOOL)muted forAppWithBundleID:(NSString *)bundleID {
    ...
    // TODO: Implement
    NSLog(@"[Flo] Set muted %d for %@", muted, bundleID);
...
- (BOOL)setPan:(float)pan forAppWithBundleID:(NSString *)bundleID {
    ...
    // TODO: Implement
    NSLog(@"[Flo] Set pan %.2f for %@", pan, bundleID);
    return YES;
}
    ...
    AudioObjectAddPropertyListenerBlock(
        ...
        ^(UInt32 inNumberAddresses, const AudioObjectPropertyAddress *inAddresses) {
            ...
            [[NSNotificationCenter defaultCenter] 
                postNotificationName:kFloDeviceListChangedNotification 
                object:nil];
        }
    );
}
...
BOOL FloAudioSystemInitialize(void) {
    BGMDeviceWrapper *device = [BGMDeviceWrapper sharedDevice];
    return [device connect];
}

void FloAudioSystemShutdown(void) {
    BGMDeviceWrapper *device = [BGMDeviceWrapper sharedDevice];
    [device disconnect];
}

BOOL FloIsDriverInstalled(void) {
    BGMDeviceWrapper *device = [BGMDeviceWrapper sharedDevice];
    return [device connect];
}

NSString *FloGetDriverVersion(void) {
