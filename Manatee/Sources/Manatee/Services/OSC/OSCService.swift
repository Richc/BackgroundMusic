...
final class OSCService: ObservableObject {
    ...
    /*
     ...
     /manatee/app/{bundleID}/volume      Float 0.0-1.5
     /manatee/app/{bundleID}/mute        Bool (0 or 1)
     /manatee/app/{bundleID}/pan         Float -1.0 to 1.0
     /manatee/app/{bundleID}/solo        Bool (0 or 1)
     
     /manatee/device/{uid}/volume        Float 0.0-1.0
     /manatee/device/{uid}/mute          Bool (0 or 1)
     
     /manatee/master/volume              Float 0.0-1.5
     /manatee/master/mute                Bool (0 or 1)
     
     /manatee/scene/recall               Int (scene index)
     /manatee/preset/recall              String (preset name)
     
     /manatee/query                      Requests current state
     */
    ...
    func broadcastAppVolume(bundleID: String, volume: Float) {
        let address = "/manatee/app/\(bundleID)/volume"
    ...
    func broadcastMasterVolume(_ volume: Float) {
        let address = "/manatee/master/volume"
        for clientAddress in connectedClients {
            let components = clientAddress.split(separator: ":")
            if components.count == 2,
               let port = UInt16(components[1]) {
