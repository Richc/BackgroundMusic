...
struct MenuBarIconView: View {
    ...
    private var menuBarImage: NSImage? {
        ...
        // Try 4: Check inside Manatee_Manatee.bundle
        if let resourcesURL = Bundle.main.executableURL?.deletingLastPathComponent().deletingLastPathComponent().appendingPathComponent("Resources/Manatee_Manatee.bundle") {
            let iconURL = resourcesURL.appendingPathComponent("MenuBarIcon.png")
            if FileManager.default.fileExists(atPath: iconURL.path),
               let nsImage = NSImage(contentsOf: iconURL) {
                print("✅ MenuBarIcon loaded from Manatee_Manatee.bundle: \(iconURL)")
                return configureMenuBarImage(nsImage)
            }
        }
    ...
    var body: some View {
        ...
        if let image = menuBarImage {
            ...
            // Fallback: use a water/wave system icon as it's thematically appropriate for Manatee
